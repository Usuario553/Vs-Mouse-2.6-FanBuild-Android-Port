...
IntroTagWidth = 35	--Width of the box's tag thingy.
--easy script configs
...
function onCreate()
	...
	makeGraphic('JukeBox', 300, 100, '280000')
	setObjectCamera('JukeBox', 'other')
	addLuaSprite('JukeBox', true)
	
	--the text for the "Now Playing" bit
	makeLuaText('JukeBoxText', '', 300, -305-IntroTagWidth, 30)
	...
	makeLuaText('JukeBoxSubText', songName, 300, -305-IntroTagWidth, 49)
...
function onCreate()
    makeLuaText('healthText', 'Health: ' .. math.floor(getProperty("health") * 50), 300, screenWidth / 2 - 300 / 2, screenHeight / 3.2 - 270 / 1.5)
...
function onCreate()
    makeLuaText('songText', "VS. Mouse | 2.6 Optimized by fnf mods", 0, 2, 700);
...
function onUpdate(elapsed)
    ...
    setTextString('songText', "VS. Mouse | 2.6");
end
...
function onSongStart()
    ...
    makeLuaText('shits', 'Shit:' .. getProperty('shits'), 200, 0, 450);
     makeLuaText('misses', 'Upset Disney:' .. getProperty('songMisses'), 200, 0, 475);
    addLuaText('hits');
    addLuaText('sicks');
    addLuaText('goods');
    addLuaText('bads');
...
function onRecalculateRating()
    ...
    setTextString('misses', 'Upset Disney:' .. getProperty('songMisses'));
end